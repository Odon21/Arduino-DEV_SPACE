...
void setup() {
  pinMode(pinBotaoCall, OUTPUT);
  analogWrite(pinBotaoCall, 10);
  ...
  for (int x = 0; x<15; x++){
    ...
    if (temSMS){
      ...
      temSMS=false;
        ...
        // (analogRead(pinBotaoCall)>=3400 && !callStatus)
    if ( ((digitalRead(infrared)==LOW) && !callStatus)){
      analogWrite(pinBotaoCall, 255);
      ...
      for(int i=0; i<3; i++){
        lcd.setCursor(10, 3);
        lcd.print(digitalRead(pinBotaoCall));
        lcd.setCursor(0, 0);
