...
#define numeroCall "+258833501120"//"+258833501120"
...
void loop() {
  ...
  tamagotchi();
  //lcd.setCursor(1, 0);
  //lcd.print(queueQUEUE_TYPE_COUNTING_SEMAPHORE());
  lcd.setCursor(0, 3);
  lcd.print("S1");
  lcd.setCursor(0, 1);
  ...
  for (int x = 0; x<15; x++){
    ...
    if (comandoGSM !=""){
    ...
    else if ((analogRead(pinBotaoCall)>=3400 && !callStatus) or ((digitalRead(infrared)==LOW) && !callStatus)){
      enviaSMS(numeroCall, "Alvo detectado");
      ...
      if (ultimoGSM.indexOf("ERROR")>-1){
        lcd.clear();
        lcd.setCursor(0, 0);
        lcd.print("Erro! Ligacao nao ef");
        lcd.setCursor(0, 1);
        lcd.print("etuada");
        enviaSMS(numeroCall, "Erro ao tentar realizar a chamada");
        callStatus = false;
        callS=20;
        delay(3000);
      }
    ...
    else if (ultimoGSM.indexOf("+COLP:") > -1){
    ...
    else if (ultimoGSM.indexOf("NO CARRIER") > -1){
      callS = 20;
      lcd.clear();
      lcd.setCursor(0, 0);
      ...
      enviaSMS(numeroCall, "Erro. Ligacao afetuada e nao foi atendida.");
      ultimoGSM = "";
      callStatus = false;
      delay(1500);
      lcd.clear();
    ...
    else if (ultimoGSM.indexOf("BUSY") > -1){
      callS = 20;
      lcd.clear();
      lcd.setCursor(0, 0);
    ...
    else if (ultimoGSM.indexOf("NO DIALTONE")> -1){
      callS = 20;
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print("Sm Linha");
    ...
    else if (ultimoGSM.indexOf("NO ANSWER") > -1){
      lcd.clear();
      callS = 20;
      lcd.setCursor(0, 0);
